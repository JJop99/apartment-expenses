...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = m > 0
      ? await sql`SELECT COALESCE(SUM(CASE WHEN type='expense' AND paid_by='Jacopo' THEN amount ELSE 0 END),0)::float AS jacopo_paid, COALESCE(SUM(CASE WHEN type='expense' AND paid_by='Amin' THEN amount ELSE 0 END),0)::float AS amin_paid, COALESCE(SUM(CASE WHEN type='expense' THEN jacopo_share ELSE 0 END),0)::float AS jacopo_owes, COALESCE(SUM(CASE WHEN type='expense' THEN amin_share ELSE 0 END),0)::float AS amin_owes, COALESCE(SUM(CASE WHEN type='reimbursement' AND paid_by='Jacopo' THEN amount ELSE 0 END),0)::float AS jacopo_reimbursed, COALESCE(SUM(CASE WHEN type='reimbursement' AND paid_by='Amin' THEN amount ELSE 0 END),0)::float AS amin_reimbursed FROM entries WHERE EXTRACT(MONTH FROM date)=${m} AND EXTRACT(YEAR FROM date)=${year}`
      : await sql`SELECT COALESCE(SUM(CASE WHEN type='expense' AND paid_by='Jacopo' THEN amount ELSE 0 END),0)::float AS jacopo_paid, COALESCE(SUM(CASE WHEN type='expense' AND paid_by='Amin' THEN amount ELSE 0 END),0)::float AS amin_paid, COALESCE(SUM(CASE WHEN type='expense' THEN jacopo_share ELSE 0 END),0)::float AS jacopo_owes, COALESCE(SUM(CASE WHEN type='expense' THEN amin_share ELSE 0 END),0)::float AS amin_owes, COALESCE(SUM(CASE WHEN type='reimbursement' AND paid_by='Jacopo' THEN amount ELSE 0 END),0)::float AS jacopo_reimbursed, COALESCE(SUM(CASE WHEN type='reimbursement' AND paid_by='Amin' THEN amount ELSE 0 END),0)::float AS amin_reimbursed FROM entries WHERE EXTRACT(YEAR FROM date)=${year}`
    ...
    return NextResponse.json({ byCategory: bycat, totals: { ...t, net } })
  } catch(e) {
    console.error(e)
